...
def test_image_generations_success_sets_route_headers_and_state(tmp_path, monkeypatch):
    ...
    def _fake_proxy(endpoint, upstream_path, body, headers, timeout_s):
        ...
        assert timeout_s == 2
        assert b'"size": "1024x1024"' in body
        assert headers["content-type"] == "application/json"
        return type("_Resp", (), {
            "status_code": 200,
...
def test_videos_success_sets_route_headers_and_state(tmp_path, monkeypatch):
    """Video endpoint should proxy multipart body and cleanup runtime counters."""
    config_path = tmp_path / "scheduler.yaml"
    config_path.write_text(
        textwrap.dedent(
            """
...
def test_server_shutdown_stops_instances_with_stop_config(tmp_path):
    """Server shutdown should best-effort stop managed instances."""
    config_path = tmp_path / "scheduler.yaml"
    config_path.write_text(
        textwrap.dedent(
            """
    ...
    instance_view = app.state.instance_lifecycle_manager.snapshot()["worker-0"]
    assert instance_view.process_state == "stopped"
    assert instance_view.last_operation == "stop"
