...
class ServerConfig(BaseModel):
    ...
    instance_health_check_failures_before_unhealthy: int = Field(default=3, ge=1)
...
class BaselinePolicyConfig(BaseModel):
    ...
    algorithm: str = "min_queue_length"
    runtime_profile_path: str | None = None

    @field_validator("algorithm")
    @classmethod
    def validate_algorithm(cls, value: str) -> str:
        if value not in {"min_queue_length", "round_robin", "short_queue_runtime"}:
            raise ValueError(
                "policy.baseline.algorithm must be one of: min_queue_length, round_robin, short_queue_runtime"
            )
        return value
    ...
    def validate_backends(cls, value: list[str]) -> list[str]:
        ...
        for backend in value:
            ...
            if backend_name not in SUPPORTED_BACKENDS:
                raise ValueError("instances[].backends must be one of: " + ", ".join(SUPPORTED_BACKENDS))
...
def load_config(config_path: str | Path) -> GlobalSchedulerConfig:
    """Load and validate one scheduler YAML config."""
