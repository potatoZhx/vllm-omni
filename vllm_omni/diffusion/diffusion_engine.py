...
class DiffusionEngine:
    ...
    def _plan_chunk_budget(self, request: OmniDiffusionRequest) -> int:
        ...
        if self._should_run_to_completion_for_guarded_tail_request(request):
            return remaining_steps
    ...
    def _should_run_to_completion_for_guarded_tail_request(self, request: OmniDiffusionRequest) -> bool:
        ...
        if policy == "sjf_aging_guarded_tail":
            return bool(getattr(request, "tail_protected", False)) and bool(getattr(request, "tail_hard_escape", False)) and not bool(
                getattr(request, "tail_sunk", False)
            )
