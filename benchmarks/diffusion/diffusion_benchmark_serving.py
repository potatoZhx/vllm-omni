...
def _build_warmup_requests(
    ...
    for i in range(args.warmup_requests):
        warm_req = _apply_request_profile(
            base_req=requests_list[i % len(requests_list)],
            profile={},
            fallback_num_inference_steps=args.warmup_num_inference_steps,
        )
        ...
        warmup_requests.append(warm_req)
...
def _populate_slo_ms_from_warmups(
    ...
    if getattr(args, "warmup_request_config", None):
        ...
        for req in requests_list:
            ...
            slo_ms = (
                ...
                else (estimated_cost_s * 1000.0 * slo_scale if estimated_cost_s is not None else None)
            )
    ...
    for req in requests_list:
        ...
        if estimated_cost_s is None:
            ...
            estimated_cost_s = (expected_ms / 1000.0) if expected_ms is not None else None
        slo_ms = (
            req.slo_ms
            if req.slo_ms is not None
            else (estimated_cost_s * 1000.0 * slo_scale if estimated_cost_s is not None else None)
...
def _extract_generated_media(response_body: Any, task: str) -> list[tuple[str, bytes]]:
    if isinstance(response_body, bytes):
        if task in {"t2v", "i2v", "ti2v"}:
            return [("mp4", response_body)]
        return []
...
def save_generated_outputs(
    ...
    saved_count = 0
    for req_idx, (req, out) in enumerate(zip(requests_list, outputs)):
        if not out.success:
            continue
