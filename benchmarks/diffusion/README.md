...
- `--inject-scheduler-slo`: Additionally inject `slo_ms` / `slo_target_ms` into request payloads. `estimated_cost_s` is sent whenever it is available from trace or warmup inference.
- `--save-output-dir`: Save successful generated image/video outputs into a local directory for spot checking.
