...
cases = [
    ("sjf_baseline", "sjf"),
    ("sjf_preempt", "sjf+preempt"),
    ("slo_first_baseline", "slo_first"),
    ("slo_first_preempt", "slo_first+preempt"),
]
...
summary = []
for case_name, display_name in cases:
    result_path = out_dir / case_name / f"result_{case_name.split('_')[0] if case_name.startswith('sjf') else 'slo_first'}.json"
    if case_name == "sjf_preempt":
        result_path = out_dir / case_name / "result_sjf.json"
    elif case_name == "sjf_baseline":
        result_path = out_dir / case_name / "result_sjf.json"
    elif case_name == "slo_first_baseline":
        result_path = out_dir / case_name / "result_slo_first.json"
    elif case_name == "slo_first_preempt":
        result_path = out_dir / case_name / "result_slo_first.json"
    server_log = out_dir / case_name / f"server_{case_name.split('_')[0] if case_name.startswith('sjf') else 'slo_first'}.log"
    if case_name.startswith("sjf"):
        server_log = out_dir / case_name / "server_sjf.log"
    else:
        server_log = out_dir / case_name / "server_slo_first.log"
