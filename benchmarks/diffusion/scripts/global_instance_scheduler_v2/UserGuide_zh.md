...
`run_case.sh` 只跑一个 case，最适合拿来确认：

- 端口是否对
- worker 能不能拉起
- NPU 设备划分是否对
- 模型名和启动参数是否对
- benchmark 是否能真正打到 scheduler
...
- worker 一直起不来
  先检查 `instances[*].endpoint` 是否和 worker 实际监听端口一致
- step-level case 一启动就失败
  先检查是否启用了 `--diffusion-enable-step-chunk`
- 明明想跑基线，却看起来像 step-level 路径
  先看 `global_scheduler.generated.yaml`，确认最终 worker args
- `short_queue_runtime` 看起来没有收益
  先确认是不是多 worker 场景，再确认请求是否带了 `estimated_cost_s`
